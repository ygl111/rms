...
bool MessageProcessor::Initialize(const ParserConfig& config) {
    ...
    // 初始化服务层
    device_service_ = std::make_shared<DeviceServiceImpl>(db_client_, cache_client_, mq_client_, response_generator_, ftp_config_, config_.auth);
    banknote_service_ = std::make_shared<BanknoteServiceImpl>(db_client_, cache_client_, mq_client_);
    fault_service_ = std::make_shared<FaultServiceImpl>(db_client_, cache_client_, mq_client_);
...
bool MessageProcessor::ProcessMessage(const DPower::Redis::DPowerRedisMessage& message) {
    ...
    try {
        ...
        if (!parsed_msg.is_valid) {
            Log("ERROR", "Failed to parse message: " + parsed_msg.error_message);
        ...
        // 确认消息处理
        mq_client_->AckMessage(config_.redis.request_stream_key, config_.redis.consumer_group, message.id);
        ...
        HandleError("Exception in ProcessMessage: " + std::string(e.what()));
        stats_.messages_failed++;
        return false;
    }
}

    ...
    while (std::getline(ss, item, ',')) {
        ...
        if (colon_pos != std::string::npos) {
            ...
            try {
                ...
                std::cerr << "Warning: Invalid port number in port list: " << port_str << std::endl;
            }
        } else {
            // 如果没有协议名称，使用默认协议
            try {
                ...
                std::cerr << "Warning: Invalid port number in port list: " << item << std::endl;
            }
        }
    }
...
bool Config::validate(const AppConfig& config) {
    ...
    if (config.redis_host.empty()) {
        std::cerr << "Error: Redis host is not configured" << std::endl;
        return false;
    }
    
    if (config.redis_port <= 0 || config.redis_port > 65535) {
        std::cerr << "Error: Invalid Redis port: " << config.redis_port << std::endl;
        return false;
    }
    
    if (config.redis_stream_key_request.empty()) {
        std::cerr << "Error: Redis request stream key is not configured" << std::endl;
        return false;
    }
    
    if (config.redis_queue_name_response.empty()) {
        std::cerr << "Error: Redis response queue key is not configured" << std::endl;
        return false;
    }
    
    // 检查端口配置
    if (config.listen_ports.empty()) {
        std::cerr << "Warning: No listen ports configured, using default port: " << config.default_port << std::endl;
    }
    
    // 检查性能配置
    if (config.max_connections <= 0) {
        std::cerr << "Error: Invalid max_connections: " << config.max_connections << std::endl;
        return false;
    }
    
    if (config.batch_size <= 0) {
        std::cerr << "Error: Invalid batch_size: " << config.batch_size << std::endl;
        return false;
    }
    
    if (config.queue_size <= 0) {
        std::cerr << "Error: Invalid queue_size: " << config.queue_size << std::endl;
        return false;
    }

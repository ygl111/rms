...
// 异步日志器，避免同步日志阻塞网络IO线程
class AsyncLogger {
public:
    AsyncLogger() : stop_flag_(false) {
        worker_thread_ = std::thread([this] { log_worker(); });
    }
    ...
    void log_async(const std::string& level, const std::string& message) {
...
private:
    void log_worker() {
        while (!stop_flag_.load() || !log_queue_.empty()) {
            std::unique_lock<std::mutex> lock(queue_mutex_);
            cv_.wait(lock, [this] { return !log_queue_.empty() || stop_flag_.load(); });
            
            while (!log_queue_.empty()) {
                auto message = std::move(log_queue_.front());
                log_queue_.pop();
                lock.unlock();
...
#define LOG_INFO(msg) do { \
    ...
    logger.log_async("[INFO]", msg); \
} while(0)

#define LOG_DEBUG(msg) do { \
    static AsyncLogger logger; \
    logger.log_async("[DEBUG]", msg); \
} while(0)

#define LOG_ERROR(msg) do { \
    static AsyncLogger logger; \
    logger.log_async("[ERROR]", msg); \
} while(0)

...
PacketReassembler::PacketReassembler(int timeout_seconds)
    : timeout_seconds_(timeout_seconds),
      ...
      timeout_sessions_(0),
      error_packets_(0) {
        Utils::Logger::Instance().Log("INFO", "Initialized with timeout: " + std::to_string(timeout_seconds) + " seconds", "PacketReassembler");
}

PacketReassembler::~PacketReassembler() {
    std::lock_guard<std::mutex> lock(sessions_mutex_);
        Utils::Logger::Instance().Log("INFO", "Shutting down. Active sessions: " + std::to_string(sessions_.size()), "PacketReassembler");
}
...
std::vector<uint8_t> PacketReassembler::ProcessMessage(const std::vector<uint8_t>& message_data, 
    ...
    if (packet_type == PacketType::SINGLE) {
        ...
        Utils::Logger::Instance().Log("DEBUG", "Single packet message from " + client_info, "PacketReassembler");
        return message_data;
    }
    ...
    
    {
        ...
        oss << "Multi-packet message from " << client_info
            << ", device: " << device_id
            << ", seq: " << seq_num
            << ", type: " << static_cast<int>(packet_type);
        Utils::Logger::Instance().Log("DEBUG", oss.str(), "PacketReassembler");
    }
    ...
    if (packet_type == PacketType::FIRST) {
        ...
        if (existing_it != sessions_.end()) {
            Utils::Logger::Instance().Log("WARN", "Cleaning up existing session for device: " + device_id, "PacketReassembler");
        ...
        if (session_it == sessions_.end()) {
            Utils::Logger::Instance().Log(
                "ERROR",
                "Received non-first packet without existing session: " + session_key +
    ...
    if (body_end <= body_start) {
        Utils::Logger::Instance().Log("ERROR", "Invalid message size for session: " + session_key, "PacketReassembler");
    ...
    if (packet_type == PacketType::LAST) {
        ...
        if (session->packets.size() == 2) {
            ...
            Utils::Logger::Instance().Log(
                "ERROR",
                "Invalid packet sequence length: " + std::to_string(session->packets.size()) +
        ...
        if (!sequence_valid) {
            Utils::Logger::Instance().Log("ERROR", "Invalid packet sequence for session: " + session_key, "PacketReassembler");
            sessions_.erase(sessions_.find(session_key));
            std::lock_guard<std::mutex> stats_lock(stats_mutex_);
            error_packets_++;
            return {};
        ...
        Utils::Logger::Instance().Log(
            "INFO",
            "Successfully reassembled message for session: " + session_key +
                ", total size: " + std::to_string(reassembled_message.size()) + " bytes",
            "PacketReassembler");
    ...
    Utils::Logger::Instance().Log(
        "DEBUG",
        "Waiting for more packets for session: " + session_key +
            " (" + std::to_string(session->packets.size()) + " packets received)",
        "PacketReassembler");
    return {};
}
...
void PacketReassembler::CleanupExpiredSessions() {
    ...
    while (it != sessions_.end()) {
        if (now - it->second->start_time > timeout_duration) {
            Utils::Logger::Instance().Log("WARN", "Cleaning up expired session: " + it->first, "PacketReassembler");
    ...
    if (removed_count > 0) {
        ...
        Utils::Logger::Instance().Log("INFO", "Cleaned up " + std::to_string(removed_count) + " expired sessions", "PacketReassembler");
    }
}

            ...
            std::thread([response_gen, ftp_cfg, device_id, shared_request_msg, task, mq_client, redis_queue_key]()
                ...
                try {
                    ...
                    if (push_result.success) {
                        std::cout << "[INFO] [升级推送] 异步升级指令已发送: " << device_id << std::endl;
                    } else {
                        std::cerr << "[ERROR] [升级推送] 发送失败: " << device_id << std::endl;
                    }
                } catch (const std::exception& e) {
                    std::cerr << "[ERROR] [升级推送] 异常: " << e.what() << std::endl;
                } })
                .detach();
...
void DeviceServiceImpl::Log(const std::string &level, const std::string &message) const
{
    ...
    std::string log_msg = "[" + timestamp + "] [" + level + "] [DeviceService] " + message;

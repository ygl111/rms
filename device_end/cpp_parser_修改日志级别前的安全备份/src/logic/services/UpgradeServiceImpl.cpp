    ...
    try {
        ...
        if (parsed_msg.HasField("upgrade_task_id")) {
            ...
            if (!task_id.empty()) {
                ...
                if (last != std::string::npos) task_id.erase(last + 1);
            }
            // 截断到第一个 NUL，避免隐藏字符
        ...
        if (parsed_msg.HasField("current_version")) {
            ...
            // 去除可能的尾随空格
            firmware_version.erase(firmware_version.find_last_not_of(" \t\r\n") + 1);
        ...
        if (!failure_desc.empty()) {
            // 去除可能的尾随空格
            failure_desc.erase(failure_desc.find_last_not_of(" \t\r\n") + 1);
            // 截断到第一个\0，避免写入不可见字符
        ...
        Log("INFO", "Processing upgrade result - Task Code: " + task_id + ", Result: " + upgrade_result + ", Version: " + firmware_version);

...
public:
    ...
    DPowerDBResult GetDeviceById(const std::string& device_id_str, DPowerDeviceRecord& out) override {
        ...
        bind[0].buffer_type = MYSQL_TYPE_STRING; bind[0].buffer = id_buffer; bind[0].buffer_length = sizeof(id_buffer); bind[0].length = &id_length;
        bind[1].buffer_type = MYSQL_TYPE_STRING; bind[1].buffer = device_id_buffer; bind[1].buffer_length = sizeof(device_id_buffer); bind[1].length = &device_id_length;
        bind[2].buffer_type = MYSQL_TYPE_LONG; bind[2].buffer = &device_type;
        bind[3].buffer_type = MYSQL_TYPE_STRING; bind[3].buffer = auth_code_buffer; bind[3].buffer_length = sizeof(auth_code_buffer); bind[3].length = &auth_code_length;
        bind[4].buffer_type = MYSQL_TYPE_LONG; bind[4].buffer = &model_id;
        bind[5].buffer_type = MYSQL_TYPE_STRING; bind[5].buffer = online_status_buffer; bind[5].buffer_length = sizeof(online_status_buffer); bind[5].length = &online_status_length;
        bind[6].buffer_type = MYSQL_TYPE_STRING; bind[6].buffer = firmware_version_buffer; bind[6].buffer_length = sizeof(firmware_version_buffer); bind[6].length = &firmware_version_length;
        bind[7].buffer_type = MYSQL_TYPE_STRING; bind[7].buffer = hardware_version_buffer; bind[7].buffer_length = sizeof(hardware_version_buffer); bind[7].length = &hardware_version_length;
        bind[8].buffer_type = MYSQL_TYPE_STRING; bind[8].buffer = main_software_version_buffer; bind[8].buffer_length = sizeof(main_software_version_buffer); bind[8].length = &main_software_version_length;
        bind[9].buffer_type = MYSQL_TYPE_STRING; bind[9].buffer = currency_library_version_buffer; bind[9].buffer_length = sizeof(currency_library_version_buffer); bind[9].length = &currency_library_version_length;
        bind[10].buffer_type = MYSQL_TYPE_STRING; bind[10].buffer = suffix_marker_buffer; bind[10].buffer_length = sizeof(suffix_marker_buffer); bind[10].length = &suffix_marker_length;
        bind[11].buffer_type = MYSQL_TYPE_STRING; bind[11].buffer = ip_endpoint_buffer; bind[11].buffer_length = sizeof(ip_endpoint_buffer); bind[11].length = &ip_endpoint_length;
        bind[12].buffer_type = MYSQL_TYPE_DATETIME; bind[12].buffer = &last_online_time_mysql;
        ...
        auto safe_string = [](const char* buffer, unsigned long length, size_t max_len) -> std::string {
            if (length > max_len) {
        ...
        out.device_id = safe_string(device_id_buffer, device_id_length, 64);
        out.device_type = device_type;
        out.auth_code = safe_string(auth_code_buffer, auth_code_length, 64);
        out.model_id = model_id;
        out.online_status = safe_string(online_status_buffer, online_status_length, 10);
        out.firmware_version = safe_string(firmware_version_buffer, firmware_version_length, 64);
        out.hardware_version = safe_string(hardware_version_buffer, hardware_version_length, 512);
        out.main_software_version = safe_string(main_software_version_buffer, main_software_version_length, 1024);
        out.currency_library_version = safe_string(currency_library_version_buffer, currency_library_version_length, 2048);
        out.suffix_marker = safe_string(suffix_marker_buffer, suffix_marker_length, 10);
        out.ip_endpoint = safe_string(ip_endpoint_buffer, ip_endpoint_length, 255);
    ...
    if (!stmt.BindParams({MessageUtils::NormalizeForDb(code), MessageUtils::NormalizeForDb(device_id)}) || !stmt.Execute()) {
            return {false, "Failed to bind/execute statement: " + std::string(mysql_stmt_error(stmt.GetStmt()))};
        }

...
std::vector<uint8_t> ResponseGenerator::CreateRegistrationResponse(const UniversalParsedMessage& request_msg,
    ...
    try {
        ...
        oss << "CreateRegistrationResponse seq=" << seq_num
            << " result=0x" << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(result_code)
            << std::dec << " auth_in_len=" << authentication_code.size();
        Utils::Logger::Instance().Log("DEBUG", oss.str(), "ResponseGenerator");
    } catch (...) {}
    ...
    if (!auth_code_vec.empty()) {
        ...
        try {
            Utils::Logger::Instance().Log("DEBUG", std::string("Actual auth_code used in response: ") + oss.str(), "ResponseGenerator");
        } catch (...) {}
    }
    ...
    if (!auth_code_vec.empty()) {
        ...
        Utils::Logger::Instance().Log("DEBUG", std::string("Actual auth code in response: ") + oss.str(), "ResponseGenerator");
    }
...
std::vector<uint8_t> ResponseGenerator::CreateHeartbeatResponse(const UniversalParsedMessage& request_msg, uint8_t keep_connect, uint32_t set_maintenance_threshold) {
    ...
    body_data["response_seNum"] = PackValue(seq_num, 2);
    body_data["keep_connect"] = PackValue(keep_connect, 1);
    body_data["set_maintenance_threshold"] = PackValue(set_maintenance_threshold, 4);
    
    // 从请求消息中获取协议ID，如果没有则使用默认值
    std::string protocol_id = request_msg.protocol_id.empty() ? "dp_protocol_v1" : request_msg.protocol_id;
    
    BodyBuildContext ctx; ctx.seq_num = seq_num; ctx.keep_connect = keep_connect; ctx.set_maintenance_threshold = set_maintenance_threshold;
    std::vector<uint8_t> body = CreateBody(response_msg_id, body_data, protocol_id, &ctx);
    std::vector<uint8_t> header = CreateHeader(response_msg_id, body.size(), device_unique_id, seq_num, protocol_id);
    std::vector<uint8_t> packet;
    packet.insert(packet.end(), header.begin(), header.end());
...
std::vector<uint8_t> ResponseGenerator::CreateUpgradeQueryResponse(const UniversalParsedMessage& request_msg, bool has_upgrade) {
    ...
    if (has_upgrade) {
        ...
        const bool has_window = upgrade_info_.has_schedule_window;
        std::vector<uint8_t> start_time_bytes(5, 0xFF);
        std::vector<uint8_t> end_time_bytes(5, 0xFF);
        if (has_window) {
            start_time_bytes = PackScheduleDateTime(upgrade_info_.start_date, upgrade_info_.time_arrange_start);
            end_time_bytes = PackScheduleDateTime(upgrade_info_.end_date, upgrade_info_.time_arrange_end);
        }
        body_data["start_time"] = std::move(start_time_bytes);
        body_data["end_time"] = std::move(end_time_bytes);
...
std::vector<uint8_t> ResponseGenerator::CreateUpgradePushMessage(const std::string& device_unique_id, uint16_t seq_num, const DPower::DB::DPowerUpgradeTask& upgrade_task, const ParserConfig::FtpConfig& ftp_config, const std::string& protocol_id) {
    ...
    std::vector<uint8_t> start_time_bytes(5, 0xFF);
    std::vector<uint8_t> end_time_bytes(5, 0xFF);
    if (upgrade_task.has_schedule_window) {
        start_time_bytes = PackScheduleDateTime(upgrade_task.start_date, upgrade_task.time_arrange_start);
        end_time_bytes = PackScheduleDateTime(upgrade_task.end_date, upgrade_task.time_arrange_end);
    }
    body_data["start_time"] = start_time_bytes;
    body_data["end_time"] = end_time_bytes;
        ...
        if (bodies.contains(msg_id_str)) {
            ...
            for (const auto& field : fields) {
                ...
                if (field.contains("value")) {
                    ...
                    } else if (src == "set_maintenance_threshold") {
                        size_t sz = field["size"].is_number_integer() ? static_cast<size_t>(field["size"].get<int>()) : 4;
                        append_vec(pack_val(ctx->set_maintenance_threshold, sz));
            ...
            switch (msg_id) {
                ...
                case 32770: // 终端注册应答
                    ...
                    if (body_data.count("authentication_code")) {
                        ...
                        try {
                            Utils::Logger::Instance().Log("DEBUG", std::string("Auth code inserted into response body: ") + oss.str(), "ResponseGenerator");
                        } catch (...) {}
                    }
                    break;
                case 32771: // 终端鉴权应答
...
uint16_t ResponseGenerator::ExtractSequenceNumber(const UniversalParsedMessage& msg) const {
    ...
    std::vector<uint8_t> raw = universal_parser_->DecodeBase64(msg.raw_data_base64);
    if (raw.empty()) {
        return 0;
    }
    ...
    std::string protocol_id = msg.protocol_id.empty() ? "dp_protocol_v1" : msg.protocol_id;
    json protocol_config = protocol_manager_->GetProtocolConfig(protocol_id);
    if (protocol_config.empty()) {
        protocol_config = protocol_manager_->GetProtocolConfig("dp_protocol_v1");
    }
    ...
    const auto& fields = protocol_config["header"]["fields"];
    for (const auto& field : fields) {
        if (!field.contains("name") || field["name"].get<std::string>() != "seNum") {
            continue;
        }
        if (!field.contains("offset") || !field.contains("size")) {
            break;
        }
        int offset = field["offset"].get<int>();
        int size = field["size"].get<int>();
        if (offset < 0 || size <= 0 || static_cast<size_t>(offset + size) > raw.size()) {
            break;
        }
        std::string data_type = field.contains("dataType") ? field["dataType"].get<std::string>() : std::string();
        bool little_endian = data_type.find('<') != std::string::npos;
        uint16_t value = 0;
        if (size == 1) {
            value = raw[static_cast<size_t>(offset)];
        } else {
            uint32_t tmp = 0;
            for (int i = 0; i < size; ++i) {
                size_t index = static_cast<size_t>(offset + (little_endian ? i : (size - 1 - i)));
                tmp = (tmp << 8) | raw[index];
            }
            value = static_cast<uint16_t>(tmp & 0xFFFFu);
        }
        return value;
    }
...
std::vector<uint8_t> ResponseGenerator::PackScheduleDateTime(const std::chrono::system_clock::time_point& date_point,
                                                             float time_value) const {
    static const std::vector<uint8_t> kNoSchedule(5, 0xFF);

    if (!std::isfinite(time_value) || time_value < 0.0f) {
        return kNoSchedule;
    }

    if (date_point.time_since_epoch().count() <= 0) {
        return kNoSchedule;
    }

    std::time_t raw_time = std::chrono::system_clock::to_time_t(date_point);
    std::tm local_tm{};
#if defined(_WIN32)
    ...
    auto to_bcd = [](int value) -> uint8_t {
        value = std::clamp(value, 0, 99);
        return static_cast<uint8_t>(((value / 10) << 4) | (value % 10));
    };

    const int hundred = static_cast<int>(std::lround(static_cast<double>(time_value) * 100.0));
    int hours = std::clamp(hundred / 100, 0, 23);
    int minutes = std::clamp(hundred % 100, 0, 59);

    // 如果未提供具体时间段，则回退到日期本身的时分
    if (hundred == 0 && std::abs(time_value) < 1e-4f) {
        hours = std::clamp(local_tm.tm_hour, 0, 23);
        minutes = std::clamp(local_tm.tm_min, 0, 59);
    }

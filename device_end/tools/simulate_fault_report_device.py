#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Simulate device registration/login/heartbeat and send a fault report (v1)."""
...
LOG = logging.getLogger("fault_sim_v1")
...
class DPv1MessageBuilder:
    ...
    def build_fault_report(
        ...
        header = self._build_header(10, len(body))
        packet = header + body
        return packet + self._tail(packet)


def bcd_time_now() -> bytes:
    now = datetime.utcnow()
    parts = [
        now.year % 100,
        now.month,
...
def run(args: argparse.Namespace) -> int:
    ...
    with socket.create_connection((args.host, args.port), timeout=args.timeout) as sock:
        ...
        fault_packet = builder.build_fault_report(
            event_level=args.event_level,
            event_code=args.event_code,
            event_time_bcd=bcd_time_now(),
            event_content=args.event_content,
        )
        sock.sendall(fault_packet)
        log_response(receive_dp_packet_v1(sock, args.timeout), "fault_report")

    return 0


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Simulate v1 register/login/heartbeat + fault report")
    ...
    parser.add_argument("--event-level", type=int, default=2, help="Fault level")
    parser.add_argument("--event-code", type=int, default=1001, help="Fault code")
    parser.add_argument("--event-content", default="Simulated fault report", help="Fault content")

...
void Application::Stop() {
    ...
    // Wake monitor/stats threads immediately instead of waiting for long sleep intervals.
    stop_cv_.notify_all();
...
void Application::MonitorThread() {
    ...
    while (!should_stop_) {
        ...
        if (stop_cv_.wait_for(
                lock,
                std::chrono::seconds(config_.monitor.health_check_interval),
...
void Application::StatsThread() {
    ...
    while (!should_stop_) {
        ...
        if (stop_cv_.wait_for(
                lock,
                std::chrono::seconds(config_.monitor.stats_interval),
                [this] { return should_stop_.load(); })) {
            break;
        }
        lock.unlock();
...
bool Application::InitializeLogging() {
    ...
    Utils::Logger::Instance().Initialize(
        config_.log.enable_console_log,
        config_.log.enable_file_log,
        config_.log.log_file,
        config_.log.log_level
    );
...
bool Application::InitializeComponents() {
    try {
        ...
        // --- 4. 初始化 RabbitMQ 客户端（用于工作时长 detail_id 投递） ---
        if (config_.rabbitmq.enabled) {
            Log("INFO", "Initializing RabbitMQ client...");
            auto rabbitFactory = DPower::MQ::CreateRabbitMqFactory();
            rabbitmq_client_ = rabbitFactory->Create();
            auto rabbit_conn_res = rabbitmq_client_->Connect(config_.rabbitmq.uri);
            if (!rabbit_conn_res.success) {
                Log("ERROR", "Failed to connect RabbitMQ: " + rabbit_conn_res.error_message);
                return false;
            }
            Log("INFO", "RabbitMQ client initialized successfully.");
        } else {
            Log("INFO", "RabbitMQ is disabled by config.");
        }
...
void Application::Log(const std::string& level, const std::string& message) const {
    Utils::Logger::Instance().Log(level, message, "Application");
}

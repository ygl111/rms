...
void to_json(json& j, const DPowerDeviceRecord& p) {
    ...
    j = json{
        ...
        {"auth_code_hex", safe_utf8(p.auth_code)}  // 修复：对鉴权码也应用UTF-8安全检查
        // 可根据需要添加更多需要缓存的字段
    };
}
...
void to_json(json& j, const DPowerUpgradeTask& p) {
    j = json{
        ...
        {"has_schedule_window", p.has_schedule_window},
...
void from_json(const json& j, DPowerUpgradeTask& p) {
    ...
    if (j.contains("has_schedule_window")) {
        j.at("has_schedule_window").get_to(p.has_schedule_window);
    } else {
        p.has_schedule_window = false;
    }
...
constexpr const char* kRedisCacheComponent = "RedisCache";
...
public:
    ...
    std::optional<DB::DPowerDeviceRecord> GetDevice(const std::string& device_id) override {
        ...
        try {
            ...
            Utils::Logger::Instance().Log("WARN", "Parse cached device json failed for key: " + key, kRedisCacheComponent);
    ...
    void SetDevice(const DB::DPowerDeviceRecord& record, int ttl_seconds) override {
        ...
        try {
            ...
            json_str = std::string("{") +
                       ...
                       "\"model_name\":\"" + model + "\"," +
                       "\"auth_code_hex\":\"" + code + "\"}";
            Utils::Logger::Instance().Log("WARN", std::string("Device cache json dump failed, used fallback: ") + e.what(), kRedisCacheComponent);
        }
    ...
    std::optional<std::optional<DB::DPowerUpgradeTask>> GetUpgradeTask(const std::string& device_id) override {
        ...
        try {
            ...
            Utils::Logger::Instance().Log("WARN", "Parse cached upgrade task json failed for key: " + key, kRedisCacheComponent);
            return std::nullopt; // JSON 解析失败
        }
    }
    ...
    bool ClearAllCache() override {
        try {
            ...
            if (result.success) {
                ...
                Utils::Logger::Instance().Log("ERROR", "Clear cache failed: " + result.error_message, kRedisCacheComponent);
                return false;
            }
        } catch (const std::exception& e) {
            Utils::Logger::Instance().Log("ERROR", std::string("Exception while clearing cache: ") + e.what(), kRedisCacheComponent);
            return false;
        }
    }
    ...
    bool ClearAllUpgradeLocks() override {
        try {
            ...
            if (!keys_result.success) {
                Utils::Logger::Instance().Log("ERROR", "Get upgrade lock keys failed: " + keys_result.error_message, kRedisCacheComponent);
                return false;
            }
            ...
            if (del_result.success) {
                ...
                Utils::Logger::Instance().Log("ERROR", "Delete upgrade locks failed: " + del_result.error_message, kRedisCacheComponent);
                return false;
            }
        } catch (const std::exception& e) {
            Utils::Logger::Instance().Log("ERROR", std::string("Exception while clearing upgrade locks: ") + e.what(), kRedisCacheComponent);
            return false;
        }
    }
    ...
    std::string GetKey(const std::string& key) const override {
        try {
            return redis_->GetKey(key);
        } catch (const std::exception& e) {
            Utils::Logger::Instance().Log("ERROR", std::string("Exception while getting key '") + key + "': " + e.what(), kRedisCacheComponent);
            return "";
        }
    }
    ...
    bool KeyExists(const std::string& key) const override {
        try {
            ...
            Utils::Logger::Instance().Log("ERROR", std::string("Exception while checking key existence '") + key + "': " + e.what(), kRedisCacheComponent);
            return false;
        }
    }
    ...
    void SetKey(const std::string& key, const std::string& value, int ttl_seconds) override {
        try {
            ...
            if (!result.success) {
                Utils::Logger::Instance().Log("ERROR", "Set key failed for '" + key + "': " + result.error_message, kRedisCacheComponent);
            }
        } catch (const std::exception& e) {
            Utils::Logger::Instance().Log("ERROR", std::string("Exception while setting key '") + key + "': " + e.what(), kRedisCacheComponent);
        }
    }

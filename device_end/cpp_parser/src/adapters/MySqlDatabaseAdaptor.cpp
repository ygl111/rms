...
private:
    struct CacheKey {
        ...
        bool operator==(const CacheKey& other) const {
            return conn == other.conn && sql == other.sql;
        }
    };

    struct CacheKeyHasher {
        std::size_t operator()(const CacheKey& k) const {
            std::size_t h1 = std::hash<MYSQL*>{}(k.conn);
            std::size_t h2 = std::hash<std::string>{}(k.sql);
            return h1 ^ (h2 + 0x9e3779b97f4a7c15ULL + (h1 << 6) + (h1 >> 2));
        }
    };

    static thread_local std::unordered_map<CacheKey, MYSQL_STMT*, CacheKeyHasher> stmt_cache_;
    static thread_local bool stmt_cache_inited_;
    static thread_local bool stmt_cache_enabled_;

    static bool IsStmtCacheEnabled() {
        if (!stmt_cache_inited_) {
            const char* v = std::getenv("DP_DB_STMT_CACHE");
            if (!v || v[0] == '\0') {
                stmt_cache_enabled_ = true;
            } else {
                char c = static_cast<char>(std::tolower(static_cast<unsigned char>(v[0])));
                stmt_cache_enabled_ = (c == '1' || c == 'y' || c == 't');
            }
            stmt_cache_inited_ = true;
        }
        return stmt_cache_enabled_;
    }

    MYSQL* conn_;
    MYSQL_STMT* stmt_;
    bool stmt_from_cache_;
...
public:
    PreparedStatement(MYSQL* conn) : conn_(conn), stmt_(nullptr), stmt_from_cache_(false) {}
    
    ~PreparedStatement() {
        if (stmt_ && !stmt_from_cache_) {
            mysql_stmt_close(stmt_);
        }
    }

    static void ClearCachedStatementsForConn(MYSQL* conn) {
        if (!conn) {
            return;
        }
        for (auto it = stmt_cache_.begin(); it != stmt_cache_.end(); ) {
            if (it->first.conn == conn) {
                if (it->second) {
                    mysql_stmt_close(it->second);
                }
                it = stmt_cache_.erase(it);
            } else {
                ++it;
            }
        }
    ...
    bool Prepare(const std::string& sql) {
        stmt_ = nullptr;
        stmt_from_cache_ = false;

        if (IsStmtCacheEnabled()) {
            CacheKey key{conn_, sql};
            auto it = stmt_cache_.find(key);
            if (it != stmt_cache_.end() && it->second) {
                stmt_ = it->second;
                stmt_from_cache_ = true;
            ...
            if (!stmt_) {
                MYSQL_STMT* fresh = mysql_stmt_init(conn_);
                if (!fresh) {
                    return false;
                }
                if (mysql_stmt_prepare(fresh, sql.c_str(), sql.length()) != 0) {
                    std::cerr << "mysql_stmt_prepare failed: " << mysql_stmt_error(fresh) << std::endl;
                    mysql_stmt_close(fresh);
                    return false;
                }
                stmt_cache_.emplace(std::move(key), fresh);
                stmt_ = fresh;
                stmt_from_cache_ = true;
            }
            return true;
...
public:
    ...
    void Disconnect() override {
        ...
        for (auto* c : all_conns_) {
            if (c) {
                PreparedStatement::ClearCachedStatementsForConn(c);
                mysql_close(c);
            }
        }
        all_conns_.clear();
        tls_conn_ = nullptr;
        if (conn_) {
            PreparedStatement::ClearCachedStatementsForConn(conn_);
            mysql_close(conn_);
            conn_ = nullptr;
        }

        // 连接关闭时清空查询缓存，避免重连后读取到陈旧数据。
        {
            std::lock_guard<std::mutex> cache_lock(query_cache_mutex_);
            device_uuid_cache_.clear();
            device_denorm_cache_.clear();
            upgrade_task_id_cache_.clear();
        }
    }
    ...
    DPowerDBResult GetDeviceById(const std::string& device_id_str, DPowerDeviceRecord& out) override {
        ...
        const std::string sql =
            "SELECT d.id, d.device_id, d.device_type, d.authentication_code, d.model_id, "
            "d.online_status, d.firmware_version, d.hardware_version, d.main_software_version, "
            "d.currency_library_version, d.suffix_marker, d.ip_endpoint, d.last_online_time, "
            "d.maintenance_threshold, m.model_name "
            "FROM devices d "
            "LEFT JOIN device_mapping_model m ON d.model_id = m.id AND m.is_deleted = 0 "
        ...
        char id_buffer[37], device_id_buffer[65], auth_code_buffer[128], online_status_buffer[16], firmware_version_buffer[128], 
            ...
            ip_endpoint_buffer[512], model_name_buffer[128];
        ...
        unsigned long id_length, device_id_length, auth_code_length, online_status_length, firmware_version_length,
                      ...
                      ip_endpoint_length, model_name_length;
        bool id_is_null, device_id_is_null, auth_code_is_null, online_status_is_null, firmware_version_is_null,
            ...
            ip_endpoint_is_null, last_online_time_is_null, maintenance_threshold_is_null, model_name_is_null;
        bool field_error[15]; memset(field_error, 0, sizeof(field_error));
        ...
        bind[14].buffer_type = MYSQL_TYPE_STRING; bind[14].buffer = model_name_buffer; bind[14].buffer_length = sizeof(model_name_buffer); bind[14].length = &model_name_length; bind[14].is_null = &model_name_is_null;
        for (int i = 0; i < 15; ++i) {
        ...
        out.model_name = safe_string(model_name_buffer, model_name_length, 127, model_name_is_null);
    ...
    DPowerDBResult SaveBanknoteReport(const BanknoteCountReport& report) override {
        ...
        // 幂等校验：同一设备在同一count_time的点钞上报只保留一条。
        // 命中后直接返回成功，避免因消息重放导致重复入库。
        const std::string idempotent_sql =
            "SELECT id FROM banknote_counts "
            "WHERE device_id = ? AND count_time = ? AND is_deleted = 0 "
            "LIMIT 1";
        PreparedStatement idem_stmt(currentConn());
        if (!idem_stmt.Prepare(idempotent_sql) || !idem_stmt.BindParams({device_uuid, report.count_time})) {
            return {false, "Failed to prepare/bind idempotency statement"};
        }
        PreparedResult idem_result = idem_stmt.ExecuteQuery();
        if (!idem_result.success || !idem_result.result) {
            if (idem_result.result) mysql_free_result(idem_result.result);
            mysql_stmt_free_result(idem_stmt.GetStmt());
            return {false, "Failed to execute idempotency query: " + idem_result.error_message};
        }

        MYSQL_BIND idem_bind;
        memset(&idem_bind, 0, sizeof(idem_bind));
        char existing_id[37] = {0};
        unsigned long existing_id_len = 0;
        idem_bind.buffer_type = MYSQL_TYPE_STRING;
        idem_bind.buffer = existing_id;
        idem_bind.buffer_length = sizeof(existing_id);
        idem_bind.length = &existing_id_len;

        if (mysql_stmt_bind_result(idem_stmt.GetStmt(), &idem_bind) != 0) {
            if (idem_result.result) mysql_free_result(idem_result.result);
            mysql_stmt_free_result(idem_stmt.GetStmt());
            return {false, "Failed to bind idempotency query result: " + std::string(mysql_stmt_error(idem_stmt.GetStmt()))};
        }

        int idem_fetch = mysql_stmt_fetch(idem_stmt.GetStmt());
        if (idem_fetch == 0) {
            std::string existed_id(existing_id, existing_id_len);
            if (idem_result.result) mysql_free_result(idem_result.result);
            mysql_stmt_free_result(idem_stmt.GetStmt());
            return {true, "Idempotent hit, existing banknote_count id=" + existed_id};
        }
        ...
        // 2. Batch insert into banknote_count_currencies
        if (!report.currency_stats.empty()) {
            // Each row has 5 placeholders; keep batches moderate for SQL size and parsing cost.
            const size_t currency_batch_size = 500;
            for (size_t i = 0; i < report.currency_stats.size(); i += currency_batch_size) {
                const size_t current_batch = std::min(currency_batch_size, report.currency_stats.size() - i);

                std::stringstream sql_ss;
                sql_ss << "INSERT INTO banknote_count_currencies "
                          "(id, count_id, currency_code, value, note_count, amount, is_deleted) VALUES ";

                std::vector<PreparedParam> batch_params;
                batch_params.reserve(current_batch * 5);

                for (size_t j = 0; j < current_batch; ++j) {
                    if (j > 0) sql_ss << ", ";
                    sql_ss << "(UUID(), ?, ?, ?, ?, ?, 0)";

                    const auto& stat = report.currency_stats[i + j];
                    batch_params.emplace_back(report_uuid);
                    batch_params.emplace_back(MessageUtils::NormalizeForDb(stat.currency_code));
                    batch_params.emplace_back(stat.value);
                    batch_params.emplace_back(static_cast<int>(stat.note_count));
                    batch_params.emplace_back(stat.amount);
                }

                PreparedStatement currency_stmt(currentConn());
                if (!currency_stmt.Prepare(sql_ss.str())) {
                    mysql_query(currentConn(), "ROLLBACK");
                    return {false, "Failed to prepare batch currency statement: " + std::string(mysql_error(currentConn()))};
                }

                if (!currency_stmt.BindParams(batch_params) || !currency_stmt.Execute()) {
                    mysql_query(currentConn(), "ROLLBACK");
                    return {false, "Failed to execute batch currency insert: " + std::string(mysql_stmt_error(currency_stmt.GetStmt()))};
                }
            }
        }

        // 3. Batch insert into banknote_detailed_data
        if (!report.details.empty()) {
            // Each detail row has 10 placeholders; MySQL max placeholders is 65535.
            // 500 rows => 5000 placeholders, which is safe and performant.
            const size_t detail_batch_size = 500;
            for (size_t i = 0; i < report.details.size(); i += detail_batch_size) {
                const size_t current_batch = std::min(detail_batch_size, report.details.size() - i);

                std::stringstream sql_ss;
                sql_ss << "INSERT INTO banknote_detailed_data "
                          "(count_id, seq, currency_code, note_value, note_version, error_group, error_type, error_code, serial_number, stacker, is_deleted) VALUES ";

                std::vector<PreparedParam> batch_params;
                batch_params.reserve(current_batch * 10);

                for (size_t j = 0; j < current_batch; ++j) {
                    if (j > 0) sql_ss << ", ";
                    sql_ss << "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)";

                    const auto& detail = report.details[i + j];
                    const int seq = detail.seq > 0
                                        ? static_cast<int>(detail.seq)
                                        : static_cast<int>(i + j + 1);
                    batch_params.emplace_back(report_uuid);
                    batch_params.emplace_back(seq);
                    batch_params.emplace_back(MessageUtils::NormalizeForDb(detail.currency_code));
                    batch_params.emplace_back(detail.note_value);
                    batch_params.emplace_back(static_cast<int>(detail.note_version));
                    batch_params.emplace_back(static_cast<int>(detail.error_group));
                    batch_params.emplace_back(static_cast<int>(detail.error_type));
                    batch_params.emplace_back(MessageUtils::NormalizeForDb(detail.error_code));
                    batch_params.emplace_back(MessageUtils::NormalizeForDb(detail.serial_number));
                    batch_params.emplace_back(static_cast<int>(detail.stacker));
                }

                PreparedStatement detail_stmt(currentConn());
                if (!detail_stmt.Prepare(sql_ss.str())) {
                    mysql_query(currentConn(), "ROLLBACK");
                    return {false, "Failed to prepare batch detail statement: " + std::string(mysql_error(currentConn()))};
                }

                if (!detail_stmt.BindParams(batch_params) || !detail_stmt.Execute()) {
                    mysql_query(currentConn(), "ROLLBACK");
                    return {false, "Failed to execute batch detail insert: " + std::string(mysql_stmt_error(detail_stmt.GetStmt()))};
                }
            }
        }
    ...
    DPowerDBResult InsertDeviceWorkTimeDetail(DeviceWorkTimeDetail& detail) override {
        if (!ensureConnection()) return {false, "Database connection is not available."};
        ...
        // 反查实际明细ID：覆盖新插入和ON DUPLICATE UPDATE两种路径
        const std::string query_id_sql =
            "SELECT id FROM device_work_time_detail "
            "WHERE device_id = ? AND event_time_utc = ? AND is_deleted = 0 "
            "ORDER BY created_at DESC LIMIT 1";

        PreparedStatement id_stmt(currentConn());
        if (!id_stmt.Prepare(query_id_sql) || !id_stmt.BindParams({detail.device_id, std::string(time_buf)})) {
            return {false, "Failed to prepare/bind detail id query"};
        }
        PreparedResult id_result = id_stmt.ExecuteQuery();
        if (!id_result.success || !id_result.result) {
            if (id_result.result) mysql_free_result(id_result.result);
            mysql_stmt_free_result(id_stmt.GetStmt());
            return {false, "Failed to query inserted detail id: " + id_result.error_message};
        }
        ...
        bind.buffer_type = MYSQL_TYPE_STRING;
        bind.buffer = id_buffer;
        bind.buffer_length = sizeof(id_buffer);
        bind.length = &id_length;

        if (mysql_stmt_bind_result(id_stmt.GetStmt(), &bind) != 0) {
            if (id_result.result) mysql_free_result(id_result.result);
            mysql_stmt_free_result(id_stmt.GetStmt());
            return {false, "Failed to bind detail id query result: " + std::string(mysql_stmt_error(id_stmt.GetStmt()))};
        }

        if (mysql_stmt_fetch(id_stmt.GetStmt()) != 0) {
            if (id_result.result) mysql_free_result(id_result.result);
            mysql_stmt_free_result(id_stmt.GetStmt());
            return {false, "No detail id found after insert/update"};
        }

        detail.detail_id.assign(id_buffer, id_length);
    ...
    std::pair<DPowerDBResult, std::string> GetUpgradeTaskIdByCode(const std::string& task_code) override {
        const std::string normalized_task_code = MessageUtils::NormalizeForDb(task_code);
        if (auto cached = getUpgradeTaskIdCache(normalized_task_code); cached.has_value()) {
            return {{true, ""}, *cached};
        }
    ...
    PreparedStatement stmt(currentConn());
    if (!stmt.Prepare(sql) || !stmt.BindParams({normalized_task_code})) {
        ...
        setUpgradeTaskIdCache(normalized_task_code, task_uuid);
...
private:
    static constexpr int kDeviceUuidCacheTtlSec = 600;
    static constexpr int kDeviceDenormCacheTtlSec = 300;
    static constexpr int kUpgradeTaskIdCacheTtlSec = 120;
    ...
    struct DeviceUuidCacheEntry {
        std::optional<std::string> value;
        std::chrono::steady_clock::time_point expire_at;
    };

    struct DeviceDenormCacheEntry {
        std::optional<DeviceDenormInfo> value;
        std::chrono::steady_clock::time_point expire_at;
    };

    struct UpgradeTaskIdCacheEntry {
        std::string value;
        std::chrono::steady_clock::time_point expire_at;
    };
    ...
    std::unordered_map<std::string, DeviceUuidCacheEntry> device_uuid_cache_;
    std::unordered_map<std::string, DeviceDenormCacheEntry> device_denorm_cache_;
    std::unordered_map<std::string, UpgradeTaskIdCacheEntry> upgrade_task_id_cache_;

    std::optional<std::string> getDeviceUuidCache(const std::string& device_id_str) {
        std::lock_guard<std::mutex> lock(query_cache_mutex_);
        auto it = device_uuid_cache_.find(device_id_str);
        if (it == device_uuid_cache_.end()) {
            return std::nullopt;
        }
        if (std::chrono::steady_clock::now() > it->second.expire_at) {
            device_uuid_cache_.erase(it);
            return std::nullopt;
        }
        return it->second.value;
    }

    void setDeviceUuidCache(const std::string& device_id_str, const std::optional<std::string>& value) {
        std::lock_guard<std::mutex> lock(query_cache_mutex_);
        device_uuid_cache_[device_id_str] = DeviceUuidCacheEntry{
            value,
            std::chrono::steady_clock::now() + std::chrono::seconds(kDeviceUuidCacheTtlSec)
        };
    }

    std::optional<std::optional<DeviceDenormInfo>> getDeviceDenormCache(const std::string& device_id_str) {
        std::lock_guard<std::mutex> lock(query_cache_mutex_);
        auto it = device_denorm_cache_.find(device_id_str);
        if (it == device_denorm_cache_.end()) {
            return std::nullopt;
        }
        if (std::chrono::steady_clock::now() > it->second.expire_at) {
            device_denorm_cache_.erase(it);
            return std::nullopt;
        }
        return it->second.value;
    }

    void setDeviceDenormCache(const std::string& device_id_str, const std::optional<DeviceDenormInfo>& value) {
        std::lock_guard<std::mutex> lock(query_cache_mutex_);
        device_denorm_cache_[device_id_str] = DeviceDenormCacheEntry{
            value,
            std::chrono::steady_clock::now() + std::chrono::seconds(kDeviceDenormCacheTtlSec)
        };
    }

    std::optional<std::string> getUpgradeTaskIdCache(const std::string& task_code) {
        std::lock_guard<std::mutex> lock(query_cache_mutex_);
        auto it = upgrade_task_id_cache_.find(task_code);
        if (it == upgrade_task_id_cache_.end()) {
            return std::nullopt;
        }
        if (std::chrono::steady_clock::now() > it->second.expire_at) {
            upgrade_task_id_cache_.erase(it);
            return std::nullopt;
        }
        return it->second.value;
    }

    void setUpgradeTaskIdCache(const std::string& task_code, const std::string& value) {
        std::lock_guard<std::mutex> lock(query_cache_mutex_);
        upgrade_task_id_cache_[task_code] = UpgradeTaskIdCacheEntry{
            value,
            std::chrono::steady_clock::now() + std::chrono::seconds(kUpgradeTaskIdCacheTtlSec)
        };
    }
    ...
    if (auto cached = getDeviceUuidCache(device_id_str); cached.has_value()) {
            return *cached;
        }
    ...
    PreparedStatement stmt(currentConn());
        if (!stmt.Prepare(sql) || !stmt.BindParams({device_id_str})) {
            setDeviceUuidCache(device_id_str, std::nullopt);
            return std::nullopt;
        }
        ...
        if (mysql_stmt_fetch(stmt.GetStmt()) == 0) {
            ...
            if (result.result) mysql_free_result(result.result);
            mysql_stmt_free_result(stmt.GetStmt());
            setDeviceUuidCache(device_id_str, uuid);
            return uuid;
        }
    ...
    std::optional<DeviceDenormInfo> getDeviceDenormInfo(const std::string& device_id_str) {
        if (auto cached = getDeviceDenormCache(device_id_str); cached.has_value()) {
            return *cached;
        }
        ...
        PreparedStatement stmt(currentConn());
        if (!stmt.Prepare(sql) || !stmt.BindParams({device_id_str})) {
            setDeviceDenormCache(device_id_str, std::nullopt);
            return std::nullopt;
        }
        ...
        if (mysql_stmt_fetch(stmt.GetStmt()) == 0 && !device_uuid_is_null) {
            ...
            if (result.result) mysql_free_result(result.result);
            mysql_stmt_free_result(stmt.GetStmt());
            setDeviceDenormCache(device_id_str, info);
            return info;
        }
    ...
    bool ensureConnection() {
        ...
        if (tls_conn_) {
            ...
            PreparedStatement::ClearCachedStatementsForConn(tls_conn_);
...
thread_local std::unordered_map<DPower::DB::PreparedStatement::CacheKey, MYSQL_STMT*, DPower::DB::PreparedStatement::CacheKeyHasher>
    DPower::DB::PreparedStatement::stmt_cache_;
thread_local bool DPower::DB::PreparedStatement::stmt_cache_inited_ = false;
thread_local bool DPower::DB::PreparedStatement::stmt_cache_enabled_ = true;

...
static std::string StripControlChars(std::string s)
{
    s.erase(
...
DeviceServiceImpl::DeviceServiceImpl(
    ...
    const ParserConfig::FtpConfig &ftp_config,
        ...
        int offline_detection_poll_interval_seconds,
        int offline_detection_timeout_seconds)
    : db_client_(std::move(db_client)),
      ...
      ftp_config_(ftp_config),
            ...
            offline_detection_poll_interval_(std::max(1, offline_detection_poll_interval_seconds)),
            heartbeat_timeout_(std::max(1, offline_detection_timeout_seconds))
{
    if (!db_client_)
    {
...
RegistrationResult DeviceServiceImpl::RegisterDevice(const UniversalParsedMessage &parsed_msg)
{
    ...
    const std::string device_model_raw = parsed_msg.GetField<std::string>("deviceModel");
    const std::string device_model_clean = MessageUtils::CleanString(device_model_raw);
    const std::string device_model = StripControlChars(device_model_clean);
    if (device_model_raw != device_model)
    {
        Log("DEBUG", "Normalized device_model from ['" + device_model_raw + "'] to ['" + device_model + "']");
    }
    ...
    if (cached_record_opt)
    {
        ...
        const std::string cache_model_raw = record.model_name;
        const std::string cache_model_clean = MessageUtils::CleanString(cache_model_raw);
        const std::string cache_model = StripControlChars(cache_model_clean);
        if (cache_model_raw != cache_model)
        {
            Log("DEBUG", "Normalized cached device_model from ['" + cache_model_raw + "'] to ['" + cache_model + "']");
        }

        if (cache_model.empty())
        {
            Log("WARN", "Device model missing in cache for device: " + device_id + ", blocking registration.");
            return {false, "Device model mismatch", {}, 0x03};
        }

        if (cache_model != device_model)
        {
            Log("WARN", "Device model mismatch (from cache). expected=" + cache_model + ", got=" + device_model);
            Log("WARN", "Device model mismatch detail (cache). expected_raw_hex=" + ToHex(cache_model_raw) + ", got_raw_hex=" + ToHex(device_model_raw));
            return {false, "Device model mismatch", {}, 0x03};
        }
        record.model_name = cache_model;
    ...
    try
    {
        ...
        if (device_exists_in_db)
        {
            ...
            const std::string db_model_raw = db_record.model_name;
            const std::string db_model_clean = MessageUtils::CleanString(db_model_raw);
            const std::string db_model = StripControlChars(db_model_clean);
            if (db_model_raw != db_model)
            {
                Log("DEBUG", "Normalized database device_model from ['" + db_model_raw + "'] to ['" + db_model + "']");
            }
            ...
            if (db_model != device_model)
            {
                Log("WARN", "Device model mismatch (from database). expected=" + db_model + ", got=" + device_model);
                Log("WARN", "Device model mismatch detail (database). expected_raw_hex=" + ToHex(db_model_raw) + ", got_raw_hex=" + ToHex(device_model_raw));
                return {false, "Device model mismatch", {}, 0x03};
            }
            // 使用规范化后的型号值，避免重复出现填充值
            db_record.model_name = db_model;
...
std::vector<uint8_t> DeviceServiceImpl::ProcessHeartbeat(const UniversalParsedMessage &parsed_msg)
{
    ...
    // 获取设备上发的当前维护阈值
    uint32_t current_maintenance_threshold = 0xFFFFFFFF;
    try {
        current_maintenance_threshold = parsed_msg.GetField<uint32_t>("current_maintenance_threshold");
    } catch (...) {
        // 旧版本设备可能不携带此字段，使用默认值0xFFFFFFFF
        current_maintenance_threshold = 0xFFFFFFFF;
    }
    
    // 要返回给设备的维护阈值，默认为0xFFFFFFFF表示未设置
    uint32_t set_maintenance_threshold = 0xFFFFFFFF;
    
    // 如果设备上发的维护阈值不为0xFFFFFFFF，则查询数据库获取设置的维护阈值
    if (current_maintenance_threshold != 0xFFFFFFFF) {
        Log("DEBUG", "Device " + device_id + " reports current maintenance threshold: " + std::to_string(current_maintenance_threshold));
        
        // 查询数据库获取设备的维护阈值
        if (db_client_ && db_client_->IsConnected()) {
            DPower::DB::DPowerDeviceRecord device_record;
            auto db_result = db_client_->GetDeviceById(device_id, device_record);
            if (db_result.success && !device_record.device_id.empty()) {
                uint32_t db_threshold = device_record.maintenance_threshold;
                // 检查阈值是否在有效区间 [10000, 5000000]
                if (db_threshold >= 10000 && db_threshold <= 5000000) {
                    set_maintenance_threshold = db_threshold;
                    Log("DEBUG", "Device " + device_id + " maintenance threshold from DB: " + std::to_string(set_maintenance_threshold));
                } else if (db_threshold == 0) {
                    // 数据库中未设置（默认值0），返回0xFFFFFFFF表示未设置
                    set_maintenance_threshold = 0xFFFFFFFF;
                    Log("DEBUG", "Device " + device_id + " has no maintenance threshold configured in DB");
                } else {
                    // 阈值不在有效区间，返回0xFFFFFFFF
                    set_maintenance_threshold = 0xFFFFFFFF;
                    Log("WARN", "Device " + device_id + " has invalid maintenance threshold in DB: " + std::to_string(db_threshold) + " (valid range: [10000, 5000000])");
                }
            } else {
                Log("WARN", "Failed to query maintenance threshold for device: " + device_id);
            }
        }
    }
    ...
    if (task_opt.has_value())
    {
        ...
        std::string locked_task_id = cache_client_->GetKey(lock_key);
        bool is_new_task = locked_task_id.empty() || locked_task_id != task_opt->task_id;

        if (is_new_task)
        {
            if (!locked_task_id.empty())
            {
                Log("INFO", "[升级准备] 发现新任务(task_id已变更)，覆盖旧锁后推送。旧task_id: " + locked_task_id + ", 新task_id: " + task_opt->task_id);
            }
            else
            {
            ...
            return response_generator_->CreateHeartbeatResponse(parsed_msg, 1, set_maintenance_threshold);
        }
        else
        {
            Log("DEBUG", "Upgrade task found for " + device_id + ", push already locked for task [code=" + task_opt->task_code + ", id=" + locked_task_id + "]. Sending normal heartbeat.");
        }
    }
    else
    {
    ...
    return response_generator_->CreateHeartbeatResponse(parsed_msg, 0, set_maintenance_threshold);
}
...
void DeviceServiceImpl::InitializeOnlineDevices()
{
    Log("INFO", "Initializing online devices from database");
    
    if (!db_client_ || !db_client_->IsConnected())
    {
        Log("ERROR", "Database client not available. Cannot initialize online devices.");
        return;
    }
    
    auto result = db_client_->GetOnlineDevices();
    if (!result.first.success)
    {
        Log("ERROR", "Failed to load online devices from database: " + result.first.error_message);
        return;
    }
    ...
    for (const auto& device_pair : result.second)
    {
        const std::string& device_id = device_pair.first;
        const auto& last_online_time = device_pair.second;
        
        // 【修复】检查时间戳是否已超时，如果超时则给10秒宽限期让设备重新上线
        auto time_diff = now - last_online_time;
        if (time_diff > heartbeat_timeout_)
        {
            // 设备已超时，给予10秒宽限期：将时间戳设为"超时阈值-10秒"
            auto grace_time = now - heartbeat_timeout_ + std::chrono::seconds(10);
            device_last_heartbeat_[device_id] = grace_time;
            grace_period_count++;
            Log("DEBUG", "Device " + device_id + " loaded with grace period (last seen " + 
                std::to_string(std::chrono::duration_cast<std::chrono::seconds>(time_diff).count()) + "s ago)");
        }
        else
        {
            // 未超时，直接使用数据库时间戳
            device_last_heartbeat_[device_id] = last_online_time;
            Log("DEBUG", "Loaded online device: " + device_id + " (last_online_time from DB)");
        }
        loaded_count++;
    }
    
    Log("INFO", "Successfully loaded " + std::to_string(loaded_count) + " online devices from database (" +
        std::to_string(grace_period_count) + " with grace period)");
}

void DeviceServiceImpl::OfflineDetectionThread()
{
    const int poll_seconds = static_cast<int>(offline_detection_poll_interval_.count());
    const int sync_check_interval_loops = std::max(1, (60 + poll_seconds - 1) / poll_seconds);

    Log("INFO", "Offline detection thread started with poll_interval: " + std::to_string(poll_seconds) +
        " seconds, timeout: " + std::to_string(heartbeat_timeout_.count()) + " seconds");
    ...
    while (offline_detection_running_.load())
    {
        ...
        // 按轮询间隔推导循环次数，约每60秒执行一次双向同步检查
        sync_check_counter_++;
        bool do_sync_check = (sync_check_counter_ >= sync_check_interval_loops);
        ...
        // 处理离线设备：在实际更新前进行实时校验，避免与新心跳竞争导致误判
        for (const auto &device_id : offline_devices)
        {
            // 第二次检查时使用实时时间戳，而不是第一次检查的时间戳
            auto now_realtime = std::chrono::system_clock::now();
            bool still_offline = false;
            {
                ...
                {
                    // 使用实时时间戳进行判断
                    if (now_realtime - it->second > heartbeat_timeout_)
                    {
                        still_offline = true;
                    }
                }
            ...
            // 在写库前再以当前时间戳进行一次最终校验，进一步缩小竞态窗口
            auto now_final = std::chrono::system_clock::now();
            {
                std::lock_guard<std::mutex> lock(device_status_mutex_);
                auto it = device_last_heartbeat_.find(device_id);
                if (it != device_last_heartbeat_.end())
                {
                    // 使用最新的实时时间戳进行最终判断
                    if (!(now_final - it->second > heartbeat_timeout_))
                    {
            ...
            // 再次确认仍旧超时后再移除记录，防止在DB更新期间有新心跳到达
            {
                ...
                if (it != device_last_heartbeat_.end())
                {
                    // 【关键修复】使用最新的实时时间戳进行最终判断
                    auto now_verify = std::chrono::system_clock::now();
                    if (now_verify - it->second > heartbeat_timeout_)
                    {
                        device_last_heartbeat_.erase(it);
                        Log("INFO", "Device " + device_id + " removed from heartbeat tracking after offline update");
                    }
                    else
                    {
                        Log("WARN", "Device " + device_id + " received heartbeat during offline update, keeping online status");
                    }
                }
            }
        }
        
        // 【修复】双向同步检查：数据库中在线但内存中不存在的设备
        if (do_sync_check)
        {
            sync_check_counter_ = 0;
            Log("DEBUG", "Performing bidirectional sync check...");
            
            if (db_client_ && db_client_->IsConnected())
            {
                auto db_result = db_client_->GetOnlineDevices();
                if (db_result.first.success)
                {
                    std::vector<std::string> orphan_devices;  // 数据库在线但内存无记录的设备
                    
                    {
                        std::lock_guard<std::mutex> lock(device_status_mutex_);
                        for (const auto& db_device : db_result.second)
                        {
                            const std::string& device_id = db_device.first;
                            if (device_last_heartbeat_.find(device_id) == device_last_heartbeat_.end())
                            {
                                // 数据库显示在线，但内存中没有心跳记录
                                orphan_devices.push_back(device_id);
                            }
                        }
                    }
                    
                    // 将这些"孤儿设备"标记为离线
                    for (const auto& device_id : orphan_devices)
                    {
                        Log("WARN", "Found orphan online device in DB without heartbeat record: " + device_id + ", marking offline");
                        UpdateDeviceStatus(device_id, "offline", "");
                    }
                    
                    if (orphan_devices.empty())
                    {
                        Log("DEBUG", "Sync check completed: no orphan devices found");
                    }
                    else
                    {
                        Log("INFO", "Sync check completed: marked " + std::to_string(orphan_devices.size()) + " orphan devices as offline");
                    }
                }
                else
                {
                    Log("ERROR", "Sync check failed: " + db_result.first.error_message);
                }
            }
        }

        std::this_thread::sleep_for(offline_detection_poll_interval_);
    }

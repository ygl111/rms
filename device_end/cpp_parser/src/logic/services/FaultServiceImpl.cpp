...
FaultReportResult FaultServiceImpl::ProcessFaultReport(const UniversalParsedMessage& parsed_msg) {
    try {
        ...
        // Fault reported: mark device as under maintenance.
        auto status_result = db_client_->UpdateDeviceStatus(device_id, "maintenance", "", false);
        if (!status_result.success) {
            Log("WARN", "Failed to update device status to maintenance for device " + device_id + ": " + status_result.error_message);
        } else if (cache_client_) {

...
bool ParserConfig::Validate() const {
    ...
    if (redis.dead_letter_queue_key.empty()) {
        std::cerr << "Redis dead letter queue key cannot be empty" << std::endl;
        return false;
    }
    ...
    if (redis.process_max_retries <= 0) {
        std::cerr << "Redis process max retries must be positive" << std::endl;
        return false;
    }

    if (redis.retry_backoff_ms < 0) {
        std::cerr << "Redis retry backoff cannot be negative" << std::endl;
        return false;
    }

    // 验证RabbitMQ配置
    if (rabbitmq.enabled) {
        if (rabbitmq.uri.empty()) {
            std::cerr << "RabbitMQ uri cannot be empty when enabled" << std::endl;
            return false;
        }
        if (rabbitmq.worktime_queue.empty()) {
            std::cerr << "RabbitMQ worktime queue cannot be empty when enabled" << std::endl;
            return false;
        }
    }
    ...
    if (monitor.offline_detection_poll_interval_seconds <= 0) {
        std::cerr << "Offline detection poll interval must be positive" << std::endl;
        return false;
    }

    if (monitor.offline_detection_timeout_seconds <= 0) {
        std::cerr << "Offline detection timeout must be positive" << std::endl;
        return false;
    }
...
ParserConfig ParserConfig::GetDefault() {
    ...
    config.redis.dead_letter_queue_key = "device_raw_messages_dlq";
    config.redis.consumer_group = "parser_group";
    config.redis.consumer_name = "cpp_parser_consumer";
    config.redis.block_timeout = 100;
    config.redis.process_max_retries = 3;
    config.redis.retry_backoff_ms = 50;
    ...
    config.monitor.offline_detection_poll_interval_seconds = 5;
    config.monitor.offline_detection_timeout_seconds = 40;
    ...
    // RabbitMQ默认配置
    config.rabbitmq.enabled = false;
    config.rabbitmq.uri = "";
    config.rabbitmq.worktime_queue = "worktime_detail_queue";
    ...
    // 邮件通知 API 默认配置
    config.notification.enable_email_notification = true;
    config.notification.email_api_host = "127.0.0.1";
    config.notification.email_api_port = "80";
    config.notification.email_api_https = false;
    config.notification.email_api_verify_tls = true;
...
void ParserConfig::PrintConfig() const {
    ...
    std::cout << "  Dead Letter Queue Key: " << redis.dead_letter_queue_key << std::endl;
    ...
    std::cout << "  Process Max Retries: " << redis.process_max_retries << std::endl;
    std::cout << "  Retry Backoff: " << redis.retry_backoff_ms << "ms" << std::endl;
    ...
    std::cout << "[RabbitMQ]" << std::endl;
    std::cout << "  Enabled: " << (rabbitmq.enabled ? "Yes" : "No") << std::endl;
    std::cout << "  URI: " << (rabbitmq.uri.empty() ? "<empty>" : "***") << std::endl;
    std::cout << "  Worktime Queue: " << rabbitmq.worktime_queue << std::endl;
    ...
    std::cout << "  Offline Detection Poll Interval: " << monitor.offline_detection_poll_interval_seconds << "s" << std::endl;
    std::cout << "  Offline Detection Timeout: " << monitor.offline_detection_timeout_seconds << "s" << std::endl;
    ...
    std::cout << "  Enable Email Notification: " << (notification.enable_email_notification ? "Yes" : "No") << std::endl;
    std::cout << "  Email API Host: " << notification.email_api_host << std::endl;
    std::cout << "  Email API Port: " << notification.email_api_port << std::endl;
    std::cout << "  Email API HTTPS: " << (notification.email_api_https ? "Yes" : "No") << std::endl;
    std::cout << "  Email API Verify TLS: " << (notification.email_api_verify_tls ? "Yes" : "No") << std::endl;
...
void ParserConfig::LoadFromJson(const json& j) {
    ...
    if (j.contains("redis")) {
        ...
        if (redis_json.contains("dead_letter_queue_key")) redis.dead_letter_queue_key = redis_json["dead_letter_queue_key"];
        ...
        if (redis_json.contains("process_max_retries")) redis.process_max_retries = redis_json["process_max_retries"];
        if (redis_json.contains("retry_backoff_ms")) redis.retry_backoff_ms = redis_json["retry_backoff_ms"];
    }
    ...
    if (j.contains("rabbitmq")) {
        const auto& rabbitmq_json = j["rabbitmq"];
        if (rabbitmq_json.contains("enabled")) rabbitmq.enabled = rabbitmq_json["enabled"];
        if (rabbitmq_json.contains("uri")) rabbitmq.uri = rabbitmq_json["uri"];
        if (rabbitmq_json.contains("worktime_queue")) rabbitmq.worktime_queue = rabbitmq_json["worktime_queue"];
    }
    ...
    if (j.contains("monitor")) {
        ...
        if (monitor_json.contains("offline_detection_poll_interval_seconds")) monitor.offline_detection_poll_interval_seconds = monitor_json["offline_detection_poll_interval_seconds"];
        if (monitor_json.contains("offline_detection_timeout_seconds")) monitor.offline_detection_timeout_seconds = monitor_json["offline_detection_timeout_seconds"];
    }
    ...
    if (j.contains("notification")) {
        const auto& notification_json = j["notification"];
        if (notification_json.contains("enable_email_notification")) notification.enable_email_notification = notification_json["enable_email_notification"];
        if (notification_json.contains("email_api_host")) notification.email_api_host = notification_json["email_api_host"];
        if (notification_json.contains("email_api_port")) notification.email_api_port = notification_json["email_api_port"];
        if (notification_json.contains("email_api_https")) notification.email_api_https = notification_json["email_api_https"];
        if (notification_json.contains("email_api_verify_tls")) notification.email_api_verify_tls = notification_json["email_api_verify_tls"];
    }
}
...
json ParserConfig::ToJson() const {
    ...
    j["redis"]["dead_letter_queue_key"] = redis.dead_letter_queue_key;
    ...
    j["redis"]["process_max_retries"] = redis.process_max_retries;
    j["redis"]["retry_backoff_ms"] = redis.retry_backoff_ms;
    ...
    // RabbitMQ配置
    j["rabbitmq"]["enabled"] = rabbitmq.enabled;
    j["rabbitmq"]["uri"] = rabbitmq.uri;
    j["rabbitmq"]["worktime_queue"] = rabbitmq.worktime_queue;
    ...
    j["monitor"]["offline_detection_poll_interval_seconds"] = monitor.offline_detection_poll_interval_seconds;
    j["monitor"]["offline_detection_timeout_seconds"] = monitor.offline_detection_timeout_seconds;
    ...
    // 邮件通知 API 配置
    j["notification"]["enable_email_notification"] = notification.enable_email_notification;
    j["notification"]["email_api_host"] = notification.email_api_host;
    j["notification"]["email_api_port"] = notification.email_api_port;
    j["notification"]["email_api_https"] = notification.email_api_https;
    j["notification"]["email_api_verify_tls"] = notification.email_api_verify_tls;

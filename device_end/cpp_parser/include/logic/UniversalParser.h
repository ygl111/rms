 ...
 * 注意：协议中的 currency_statistics 只包含 币种符号、张数、金额 三个字段
 */
struct CountInfo {
    std::string currency_symbol; // 币种符号
    uint16_t count;              // 张数
    uint64_t amount;             // 金额 (单位：分)
};
...
struct NoteInfo {
    ...
    std::string error_code;      // 报错码字符串 (如"0x1902")
    std::string serial_number;   // 钞票序列号
    uint8_t stacker;             // 出钞口编号,0xFF为拒钞口
};

/**
 * @brief 字段值类型
 */
using FieldValue = std::variant<std::string, uint8_t, uint16_t, uint32_t, uint64_t, std::vector<uint8_t>, std::vector<CountInfo>, std::vector<NoteInfo>>;
...
struct FieldStructure {
    ...
    std::string size_ref;        // 变长字段的大小引用字段名
};
...
private:
    ...
    /**
     * @brief 获取复合字段的单条记录字节数
     */
    size_t GetCompositeFieldRecordSize(const std::string& field_type) const;
